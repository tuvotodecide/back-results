...
jest.mock('@/api/electoralCredits', () => ({
  CreditsContractCalls: {
    liquidate: jest.fn(),
    // 1 TVD por participante, con los 2 decimales del harness de capacidad.
    tvdPerCredit: jest.fn(async () => 100n),
  },
}));
...
describe('MX-06 TVD focal integration coverage', () => {
  ...
  it('[MX-06][TVD-RES-P0-004][INTEGRACION] usa balance blockchain como fuente autoritativa de capacidad', async () => { const service = new TvdCapacityService({ get: jest.fn().mockReturnValue(wallet) } as never, {} as never, {} as never, {} as never, {} as never, { resolveMyInstitutionalWallet: jest.fn().mockResolvedValue({ tenantId: String(tenantId), wallet }) } as never, { chain: '84532', getLiquidBalance: jest.fn().mockResolvedValue('700'), getTokenDecimals: jest.fn().mockResolvedValue(2) } as never); const result = await service.estimateCapacity('10', { sub: String(userId), tenantId: String(tenantId) }); expect(result).toMatchObject({ availableTokens: '7', estimatedRequiredTokens: '10', hasEstimatedCapacity: false, balanceSource: 'BLOCKCHAIN' }); });
